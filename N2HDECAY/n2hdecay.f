...
C          kalino@fuw.edu.pl
C          milada.muehlleitner@kit.edu
C          Michael.Spira@psi.ch
C
C
C ================ IT USES AS INPUT PARAMETERS:
C
C   SLHAIN: =0: READ FROM n2hdecay.in
...
C
C     2HDM: =0: CALCULATE USUAL BRANCHING RATIOS
C           =1: 2HDM (SETS HIGGS = 5)
C
C     N2HDM:=0: CALCULATE USUAL BRNCHING RATIOS
C           =1: N2HDM (SETS HIGGS = 5)
C
...
C ------ 2HDM ------
C TYPE:      TYPE OF 2HDM: 1 (type I), 2 (type II), 3 (lepton-specific)
C            4 (flipped)
C PARAM:     choice of input parameters: 1 (masses), 2 (lambda_i)
...
C LAMBDA1_5: LAMBDA1...LAMBDA5 FOR PARAM = 2
C ------ N2HDM ----- implementation by Jonas Wittbrodt (JW)
C takes from the 2HDM: type, tgbet2hdm, m12^2, MHA, MH+-
C needs as further inputs
C MH1, MH2, MH3: the three scalar masses
C alpha1, alpha2, alph3: the three CP-even mixing angles
C v_sing: the VEV of the singlet field
C -------------------
...
c JW changed
      if(in2hdm.eq.1.and.i2hdm.eq.0) then
            print*,'N2HDM requires some 2HDM inputs->I2HDM set to 1'
            i2hdm = 1
      endif
      if(in2hdm.eq.1.and.
     .      (AMH1_N2HDM>=AMH2_N2HDM.or.AMH2_N2HDM>=AMH3_N2HDM)) then
            print*,'!!!!!!ERROR!!!!!!'
            print*,'N2HDM scalar masses must be ordered as mH1<mH2<mH3.'
...
c JW changed
     ...
     .      ALPH3_N2HDM,AVS_N2HDM,IN2HDM
      ...
      If(in2hdm.eq.1)then
            Open(NLa,File = 'br.H1_N2HDM_a')
            Open(NLb,File = 'br.H1_N2HDM_b')
            Open(NLc,File = 'br.H1_N2HDM_c')
            Open(NHa,File = 'br.H2_N2HDM_a')
            Open(NHb,File = 'br.H2_N2HDM_b')
            Open(NHc,File = 'br.H2_N2HDM_c')
            Open(N3a,File = 'br.H3_N2HDM_a')
            Open(N3b,File = 'br.H3_N2HDM_b')
            Open(N3c,File = 'br.H3_N2HDM_c')
            Open(N3d,File = 'br.H3_N2HDM_d')
            Open(NAa,File = 'br.A_N2HDM_a')
            Open(NAb,File = 'br.A_N2HDM_b')
            Open(NAc,File = 'br.A_N2HDM_c')
            Open(NCa,File = 'br.H+_N2HDM_a')
            Open(NCb,File = 'br.H+_N2HDM_b')
            Open(NCc,File = 'br.H+_N2HDM_c')
            OPEN(NTA,FILE='br.top')

            WRITE(NLa,70)'MH1   ','BB   ','TAU TAU','MU MU ','SS ','CC '
     .           ,'TT '
            WRITE(NLa,69)
            WRITE(NLa,*)
            Write(NLb,72)'MH1   ','GG ','GAM GAM','Z GAM ','WW ','ZZ '
            WRITE(NLb,69)
            WRITE(NLb,*)
            WRITE(NLC,72)'MH1   ','AA ','Z A ','W+- H-+','H+ H- ',
     .           'WIDTH '
            WRITE(NLC,69)
            WRITE(NLC,*)

            WRITE(NHa,70)'MH2   ','BB   ','TAU TAU','MU MU ','SS ','CC '
     .           ,'TT '
            WRITE(NHa,69)
            WRITE(NHa,*)
            WRITE(NHb,72)'MH2   ','GG ','GAM GAM','Z GAM ','WW ','ZZ '
            WRITE(NHb,69)
            WRITE(NHb,*)
            WRITE(NHc,70)'MH2   ','H1H1','AA ','Z A ','W+- H-+','H+ H- '
     .           ,'WIDTH '
            WRITE(NHc,69)
            WRITE(NHc,*)

            WRITE(N3a,70)'MH3   ','BB   ','TAU TAU','MU MU ','SS ','CC '
     .           ,'TT '
            WRITE(N3a,69)
            WRITE(N3a,*)
            WRITE(N3b,72)'MH3   ','GG ','GAM GAM','Z GAM ','WW ','ZZ '
            WRITE(N3b,69)
            WRITE(N3b,*)
            WRITE(N3c,72)'MH3   ','H1H1','H1H2','H2H2','AA ','Z A '
            WRITE(N3c,69)
            WRITE(N3c,*)
            WRITE(N3d,72)'MH3   ','W+- H-+','H+ H- ','WIDTH '
            WRITE(N3d,69)
            WRITE(N3d,*)

            WRITE(NAa,70)'MHA   ','BB   ','TAU TAU','MU MU ','SS ','CC '
     .           ,'TT '
            WRITE(NAa,69)
            WRITE(NAa,*)
            WRITE(NAb,70)'MHA   ','GG ','GAM GAM','Z GAM ','Z H1 ',
     .           'Z H2','Z H3'
            WRITE(NAb,69)
            WRITE(NAb,*)
            WRITE(NAc,72)'MHA   ','W+- H-+','WIDTH '
            WRITE(NAc,69)
            WRITE(NAc,*)

            WRITE(NCA,70)'MH+   ','BC   ','TAU NU ','MU NU ','SU ','CS '
     .           ,'TB '
            WRITE(NCA,69)
            WRITE(NCA,*)
            WRITE(NCB,70)'MH+   ','CD   ','BU   ','TS   ','TD   '
            WRITE(NCB,69)
            WRITE(NCB,*)
            WRITE(NCC,72)'MH+   ','H1 W ','H2 W ','H3 W ','A W',
     .           'WIDTH '
            WRITE(NCC,69)
            WRITE(NCC,*)
      else
...
200   Format(A9,16(1X,A10))
201   Format(A9,17(1X,A10))
202   Format(A9,19(1X,A10))
203   Format(A9,14(1X,A10))
204   Format(A9,15(1x,A10))
c end JW changed
...
c JW changed
     ...
     .        hibrw(1),hibrz(1)
         write(NLc,20)amh1_n2hdm,hibraa(1),hibraz(1),hibrhpwm(1),
     .        hibrchch(1),gamtot(1)

         write(NHa,20)amh2_n2hdm,hibrb(2),hibrl(2),hibrm(2),hibrs(2),
     .        hibrc(2),hibrt(2)
         write(NHb,22)amh2_n2hdm,hibrg(2),hibrga(2),hibrzga(2),
     .        hibrw(2),hibrz(2)
         write(NHc,20)amh2_n2hdm,h2brh1h1,hibraa(2),hibraz(2),
     .        hibrhpwm(2),hibrchch(2),gamtot(2)

         write(N3a,20)amh3_n2hdm,hibrb(3),hibrl(3),hibrm(3),hibrs(3),
     .        hibrc(3),hibrt(3)
         write(N3b,22)amh3_n2hdm,hibrg(3),hibrga(3),hibrzga(3),
     .        hibrw(3),hibrz(3)
     ...
     .        hibraa(3),hibraz(3)
         ...
         write(NCC,20)amch,hcbrwh(1),hcbrwh(2),hcbrwh(3),hcbra,hcwdth
...
210   FORMAT(G12.6,16(1X,G10.4))
211   FORMAT(G12.6,17(1X,G10.4))
212   FORMAT(G12.6,19(1X,G10.4))
213   FORMAT(G12.6,14(1X,G10.4))
214   FORMAT(G12.6,15(1X,G10.4))
c end JW changed
...
c JW changed
     ...
     .      AMH3_N2HDM,ALPH1_N2HDM,ALPH2_N2HDM,
     .      ALPH3_N2HDM,AVS_N2HDM,IN2HDM
      Common/N2HDM_COUP/amhiggs,
     .      ghitop,ghibot,ghilep,
     .      ghihpwm,ghivv,ghiza,
     .      ghipm,ghiaa,ghijk
      Common/N2HDM_BR_H/h2brh1h1,h3brh1h1,h3brh2h2,h3brh1h2,
     .      hibrt,hibrb,hibrl,hibrm,hibrs,hibrc,
...
C  H ---> G G* ---> G BB   TO BE ADDED TO H ---> BB
       ...
       dbb = HVV(amhi,0.D0)*(ASG/PI)**2*XFAC/8
     .      -hgg-dcc
...
c couplings involving two gauge bosons
      ghivv(1) = ca1mb*ca2
      ghivv(2) = -(ca3*sa1mb) - ca1mb*sa2*sa3
      ghivv(3) = -(ca1mb*ca3*sa2) + sa1mb*sa3

c couplings to a W and a charged Higgs
      ghihpwm(1) = -(ca2*sa1mb)
      ghihpwm(2) = -(ca1mb*ca3) + sa1mb*sa2*sa3
      ghihpwm(3) = ca3*sa1mb*sa2 + ca1mb*sa3

c couplings to a Z and an A
      ghiza(1) = -(ca2*sa1mb)
      ghiza(2) = -(ca1mb*ca3) + sa1mb*sa2*sa3
      ghiza(3) = ca3*sa1mb*sa2 + ca1mb*sa3
...
c couplings to a pair of pseudoscalar Higgs
      ghiaa(1) = (ca2*(-2*ama**2*(sa1m3b - sa1pb) - 4*ammh2*sa1pb +
     -      am1**2*(sa1m3b + 3*sa1pb)))/(8.*cb*sb*v)
      ghiaa(2) = (-4*ammh2*ca1pb*ca3 + 4*ammh2*sa1pb*sa2*sa3 -
     -    2*ama**2*(ca1m3b*ca3 - ca1pb*ca3 +
     -       (-sa1m3b + sa1pb)*sa2*sa3) +
     -    am2**2*(ca1m3b*ca3 + 3*ca1pb*ca3 -
     -       (sa1m3b + 3*sa1pb)*sa2*sa3))/(8.*cb*sb*v)
      ghiaa(3) = (-(am3**2*(ca3*(sa1m3b + 3*sa1pb)*sa2 +
     -         (ca1m3b + 3*ca1pb)*sa3)) +
...
c only couplings relevant for decays (am1<am2<am3) are given
      ...
      ghijk(1,1,2) = (ca2*(-(((2*am1**2 + am2**2 + ammh2 +
     ...
     -            vs)/s2b)))/(16.*v*vs)
      ...
      ghijk(1,1,2) = 2*ghijk(1,1,2)

c      print*,'gh112',ghijk(1,1,2)*2.96330397762368004E-002

      ghijk(1,1,3) = (ca2*(4*ca2*((-2*am1**2 - am3**2 + 3*ammh2)*s2a1 +
     ...
     -         )))/(16.*cb*sb*v*vs)
      ...
      ghijk(1,1,3) = 2*ghijk(1,1,3)

c      print*,'g113',ghijk(1,1,3)

      ghijk(2,2,3) = (16*(2*am2**2 + am3**2)*c3a2*ca3*s2b*sa3**2*v +
     ...
     -        (3*s3a3 + 7*sa3))*vs)/(256.*cb*sb*v*vs)
      ...
      ghijk(2,2,3) = 2*ghijk(2,2,3)

c      print*,'g223',ghijk(2,2,3)

      ghijk(1,2,3) = (8*c2a3*s2a2*((-am1**2 - am2**2 - am3**2
...
c      print*,'2HDM,gzal,gzah',gzal,gzah
c      print*,'glvv,ghvv',glvv,ghvv
c      print*,'glt,glb,gllep',glt,glb,gllep
c      print*,'ght,ghb,ghlep',ght,ghb,ghlep
c      print*,'gat,gab,galep',gat,gab,galep
