double precision function H2toHpWmCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta1KanUsual() - RR21*CB*dBeta1KanUsual() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZGpHpOSUsual()/2D0 ) + &
			...
			&) * (EL2/SW2)*(CB*RR22 - RR21*SB)**2 * &
			&( (MHp2**2 + MW2**2 + MH22**2 - 2D0*MHp2*MW2 - 2D0*MHp2*MH22 - 2D0*MW2*MH22)/(4D0*MW2) )
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
			...
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZGpHpOSAlter()/2D0 ) + &
 ...
 H2toHpWmCT = totalAmplitude
end function H2toHpWmCT
