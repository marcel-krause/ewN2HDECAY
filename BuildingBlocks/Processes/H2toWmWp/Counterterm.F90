double precision function H2toWmWpCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Usual()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBeta1KanUsual() + RR22*CB*dBeta1KanUsual() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSUsual()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSUsual()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Usual()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBeta2KanUsual() + RR22*CB*dBeta2KanUsual() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSUsual()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSUsual()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Alter()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBeta1KanAlter() + RR22*CB*dBeta1KanAlter() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSAlter()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (4)
		dRR11 = -CA2*SA1*dAlpha1KanAlter() - CA1*SA2*dAlpha2KanAlter()
		dRR12 = CA1*CA2*dAlpha1KanAlter() - SA1*SA2*dAlpha2KanAlter()
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Alter()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBeta2KanAlter() + RR22*CB*dBeta2KanAlter() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSAlter()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (5)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Alter()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBeta1PinchPStar() + RR22*CB*dBeta1PinchPStar() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSAlter()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (6)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Alter()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBeta2PinchPStar() + RR22*CB*dBeta2PinchPStar() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSAlter()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (7)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Alter()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBeta1PinchOS() + RR22*CB*dBeta1PinchOS() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSAlter()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (8)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Alter()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBeta2PinchOS() + RR22*CB*dBeta2PinchOS() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSAlter()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (9)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Usual()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBetaMSBarUsual() + RR22*CB*dBetaMSBarUsual() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSUsual()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSUsual()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
	case (10)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMW2Alter()/(2D0*MW2) + dZWpWpOS() + &
			& 1D0/(CB*RR21 + RR22*SB)*( CB*dRR21 + dRR22*SB - SB*RR21*dBetaMSBarAlter() + RR22*CB*dBetaMSBarAlter() ) + &
			& (CB*RR11 + RR12*SB)/(CB*RR21 + RR22*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR21 + RR22*SB)/(CB*RR21 + RR22*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR31 + RR32*SB)/(CB*RR21 + RR22*SB)*( dZH3H2OSAlter()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR21 + RR22*SB)**2*MW2 * (3D0 + MH22**2/(4D0*MW2**2) - MH22/MW2)
 ...
 H2toWmWpCT = totalAmplitude
end function H2toWmWpCT
