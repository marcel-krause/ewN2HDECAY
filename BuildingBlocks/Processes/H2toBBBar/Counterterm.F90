double precision function H2toBBBarCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMBOSUsual()/MB - dMW2Usual()/(2D0*MW2) + dYukS1Quark2/YukS1Quark2 + &
			& dZBBOSLeft()/2D0 + dZBBOSRight()/2D0 + &
			& YukS1Quark1/YukS1Quark2*( dZH1H2OSUsual()/2D0 ) + YukS1Quark2/YukS1Quark2*( dZH2H2OS()/2D0 ) + &
			& YukS1Quark3/YukS1Quark2*( dZH3H2OSUsual()/2D0 ) &
			& )*(1.5D0*EL2*(MH22 - 4.D0*MB2)*MB2*YukS1Quark2**2)/(MW2*SW2)
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMBOSAlter()/MB - dMW2Alter()/(2D0*MW2) + dYukS1Quark2/YukS1Quark2 + &
			& dZBBOSLeft()/2D0 + dZBBOSRight()/2D0 + &
 ...
 H2toBBBarCT = totalAmplitude
end function H2toBBBarCT
