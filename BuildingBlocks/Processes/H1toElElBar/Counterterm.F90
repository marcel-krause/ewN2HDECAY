double precision function H1toElElBarCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMEOSUsual()/ME - dMW2Usual()/(2D0*MW2) + dYukS1Lep1/YukS1Lep1 + &
			...
			& )*(0.5D0*EL2*(MH12 - 4.D0*ME2)*ME2*YukS1Lep1**2)/(MW2*SW2)
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMEOSAlter()/ME - dMW2Alter()/(2D0*MW2) + dYukS1Lep1/YukS1Lep1 + &
			& dZElElOSLeft()/2D0 + dZElElOSRight()/2D0 + &
 ...
 H1toElElBarCT = totalAmplitude
end function H1toElElBarCT
