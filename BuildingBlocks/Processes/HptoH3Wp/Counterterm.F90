double precision function HptoH3WpCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBeta1KanUsual() - CB*RR31*dBeta1KanUsual() ) + &
			...
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSUsual()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR32 - RR31*SB)**2 * &
			& ( (MHp2**2 + MW2**2 + MH32**2 - 2D0*MHp2*MW2 - 2D0*MHp2*MH32 - 2D0*MW2*MH32)/(4D0*MW2) )
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBeta2KanUsual() - CB*RR31*dBeta2KanUsual() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSUsual()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSUsual()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSUsual()/2D0 ) &
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBeta1KanAlter() - CB*RR31*dBeta1KanAlter() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSAlter()/2D0 ) &
			& ) * (EL2/SW2)*(CB*RR32 - RR31*SB)**2 * &
			& ( (MHp2**2 + MW2**2 + MH32**2 - 2D0*MHp2*MW2 - 2D0*MHp2*MH32 - 2D0*MW2*MH32)/(4D0*MW2) )
	case (4)
		dRR11 = -CA2*SA1*dAlpha1KanAlter() - CA1*SA2*dAlpha2KanAlter()
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBeta2KanAlter() - CB*RR31*dBeta2KanAlter() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSAlter()/2D0 ) &
	...
	case (5)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBeta1PinchPStar() - CB*RR31*dBeta1PinchPStar() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSAlter()/2D0 ) &
	...
	case (6)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBeta2PinchPStar() - CB*RR31*dBeta2PinchPStar() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSAlter()/2D0 ) &
	...
	case (7)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBeta1PinchOS() - CB*RR31*dBeta1PinchOS() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSAlter()/2D0 ) &
	...
	case (8)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBeta2PinchOS() - CB*RR31*dBeta2PinchOS() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSAlter()/2D0 ) &
	...
	case (9)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBetaMSBarUsual() - CB*RR31*dBetaMSBarUsual() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSUsual()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSUsual()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSUsual()/2D0 ) &
	...
	case (10)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
            & 1D0/(CB*RR32 - RR31*SB)*( CB*dRR32 - SB*dRR31 - SB*RR32*dBetaMSBarAlter() - CB*RR31*dBetaMSBarAlter() ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR32 - RR31*SB)*( dZH1H3OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR32 - RR31*SB)*( dZH2H3OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR32 - RR31*SB)*( dZH3H3OS()/2D0 ) + &
			& (SB*RR32 + RR31*CB)/(CB*RR32 - RR31*SB)*( dZGpHpOSAlter()/2D0 ) &
 ...
 HptoH3WpCT = totalAmplitude
end function HptoH3WpCT
