double precision function H1toMuMuBarCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMMOSUsual()/MM - dMW2Usual()/(2D0*MW2) + dYukS1Lep1/YukS1Lep1 + &
			...
			& )*(0.5D0*EL2*(MH12 - 4.D0*MM2)*MM2*YukS1Lep1**2)/(MW2*SW2)
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMMOSAlter()/MM - dMW2Alter()/(2D0*MW2) + dYukS1Lep1/YukS1Lep1 + &
			& dZMuMuOSLeft()/2D0 + dZMuMuOSRight()/2D0 + &
 ...
 H1toMuMuBarCT = totalAmplitude
end function H1toMuMuBarCT
