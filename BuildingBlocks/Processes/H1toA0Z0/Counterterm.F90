double precision function H1toA0Z0CT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Usual()/MZ2 - dMW2Usual()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBeta1KanUsual() - RR11*CB*dBeta1KanUsual() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSUsual()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSUsual()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSUsual()/2D0 ) &
			&) * (EL2/SW2/CW2)*(CB*RR12 - RR11*SB)**2 * &
			&( (MA02**2 + MZ2**2 + MH12**2 - 2D0*MA02*MZ2 - 2D0*MA02*MH12 - 2D0*MZ2*MH12)/(4D0*MZ2) )
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Usual()/MZ2 - dMW2Usual()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBeta2KanUsual() - RR11*CB*dBeta2KanUsual() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSUsual()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSUsual()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSUsual()/2D0 ) &
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBeta1KanAlter() - RR11*CB*dBeta1KanAlter() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSAlter()/2D0 ) &
			&) * (EL2/SW2/CW2)*(CB*RR12 - RR11*SB)**2 * &
			&( (MA02**2 + MZ2**2 + MH12**2 - 2D0*MA02*MZ2 - 2D0*MA02*MH12 - 2D0*MZ2*MH12)/(4D0*MZ2) )
	case (4)
		dRR11 = -CA2*SA1*dAlpha1KanAlter() - CA1*SA2*dAlpha2KanAlter()
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBeta2KanAlter() - RR11*CB*dBeta2KanAlter() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSAlter()/2D0 ) &
	...
	case (5)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBeta1PinchPStar() - RR11*CB*dBeta1PinchPStar() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSAlter()/2D0 ) &
	...
	case (6)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBeta2PinchPStar() - RR11*CB*dBeta2PinchPStar() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSAlter()/2D0 ) &
	...
	case (7)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBeta1PinchOS() - RR11*CB*dBeta1PinchOS() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSAlter()/2D0 ) &
	...
	case (8)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBeta2PinchOS() - RR11*CB*dBeta2PinchOS() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSAlter()/2D0 ) &
	...
	case (9)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Usual()/MZ2 - dMW2Usual()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBetaMSBarUsual() - RR11*CB*dBetaMSBarUsual() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSUsual()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSUsual()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSUsual()/2D0 ) &
	...
	case (10)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR12 - RR11*SB)*( CB*dRR12 - dRR11*SB - SB*RR12*dBetaMSBarAlter() - RR11*CB*dBetaMSBarAlter() ) + &
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR12 - RR11*SB)*( dZH2H1OSAlter()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR12 - RR11*SB)*( dZH3H1OSAlter()/2D0 ) &
 ...
 H1toA0Z0CT = totalAmplitude
end function H1toA0Z0CT
