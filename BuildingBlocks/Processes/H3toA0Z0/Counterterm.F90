double precision function H3toA0Z0CT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Usual()/MZ2 - dMW2Usual()/MW2)/2D0 + &
			...
			&) * (EL2/SW2/CW2)*(CB*RR32 - RR31*SB)**2 * &
			&( (MA02**2 + MZ2**2 + MH32**2 - 2D0*MA02*MZ2 - 2D0*MA02*MH32 - 2D0*MZ2*MH32)/(4D0*MZ2) )
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
 ...
 H3toA0Z0CT = totalAmplitude
end function H3toA0Z0CT
