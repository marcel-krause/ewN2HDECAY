double precision function H1toHpWmCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
			...
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZGpHpOSUsual()/2D0 ) + &
			...
			&) * (EL2/SW2)*(CB*RR12 - RR11*SB)**2 * &
			&( (MHp2**2 + MW2**2 + MH12**2 - 2D0*MHp2*MW2 - 2D0*MHp2*MH12 - 2D0*MW2*MH12)/(4D0*MW2) )
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZHpHpOS()/2D0 + dZWpWpOS()/2D0 + &
			...
			& (SB*RR12 + RR11*CB)/(CB*RR12 - RR11*SB)*( dZGpHpOSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR12 - RR11*SB)*( dZH1H1OS()/2D0 ) + &
 ...
 H1toHpWmCT = totalAmplitude
end function H1toHpWmCT
