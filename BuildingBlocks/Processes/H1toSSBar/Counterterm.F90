double precision function H1toSSBarCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMSOSUsual()/MS - dMW2Usual()/(2D0*MW2) + dYukS1Quark1/YukS1Quark1 + &
			...
			& )*(1.5D0*EL2*(MH12 - 4.D0*MS2)*MS2*YukS1Quark1**2)/(MW2*SW2)
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMSOSAlter()/MS - dMW2Alter()/(2D0*MW2) + dYukS1Quark1/YukS1Quark1 + &
			& dZSSOSLeft()/2D0 + dZSSOSRight()/2D0 + &
 ...
 H1toSSBarCT = totalAmplitude
end function H1toSSBarCT
