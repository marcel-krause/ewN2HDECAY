double precision function H3toDDBarCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMDOSUsual()/MD - dMW2Usual()/(2D0*MW2) + dYukS1Quark3/YukS1Quark3 + &
			...
			& )*(1.5D0*EL2*(MH32 - 4.D0*MD2)*MD2*YukS1Quark3**2)/(MW2*SW2)
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMDOSAlter()/MD - dMW2Alter()/(2D0*MW2) + dYukS1Quark3/YukS1Quark3 + &
			& dZDDOSLeft()/2D0 + dZDDOSRight()/2D0 + &
 ...
 H3toDDBarCT = totalAmplitude
end function H3toDDBarCT
