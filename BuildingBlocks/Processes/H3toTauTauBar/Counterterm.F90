double precision function H3toTauTauBarCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSUsual()/ML - dMW2Usual()/(2D0*MW2) + dYukS1Lep3/YukS1Lep3 + &
			...
			& )*(0.5D0*EL2*(MH32 - 4.D0*ML2)*ML2*YukS1Lep3**2)/(MW2*SW2)
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + dYukS1Lep3/YukS1Lep3 + &
			& dZTauTauOSLeft()/2D0 + dZTauTauOSRight()/2D0 + &
 ...
 H3toTauTauBarCT = totalAmplitude
end function H3toTauTauBarCT
