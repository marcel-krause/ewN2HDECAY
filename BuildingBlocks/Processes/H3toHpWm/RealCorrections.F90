double precision function H3toHpWmReal()
 use constants
 use counterterms
 implicit none
#include "looptools.h"
...
! OsssFull = Qsss1*Qsss1*Osss11 + Qsss1*Qsss2*Osss12 + Qsss2*Qsss2*Osss22
 ...
 totalAmplitude = 1D0*(EL2*(CB*RR32 - RR31*SB)**2/SW2/4D0)*(EL2/((4D0*PI)**3*m1*m3**2))*&
     &(16D0*PI*m1**3)/DSQRT(m1**4 + m2**4 + m3**4 - 2D0*m1**2*m2**2 - 2D0*m1**2*m3**2 - 2D0*m2**2*m3**2 )*&
     &( OssvFull )

 H3toHpWmReal = totalAmplitude
end function H3toHpWmReal
