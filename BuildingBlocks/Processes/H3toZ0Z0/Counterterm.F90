double precision function H3toZ0Z0CT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMZ2Usual()/MZ2 - dMW2Usual()/(2D0*MW2) + dZZ0Z0OS() + &
			...
			& ) * (EL2/SW2)*(CB*RR31 + RR32*SB)**2*MW2/(CW2**2) * (3D0 + MH32**2/(4D0*MZ2**2) - MH32/MZ2)
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
 ...
 H3toZ0Z0CT = totalAmplitude
end function H3toZ0Z0CT
