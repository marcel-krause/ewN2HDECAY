double precision function A0toH2Z0CT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + (dMZ2Usual()/MZ2 - dMW2Usual()/MW2)/2D0 + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + &
            ...
            & * (EL2/SW2)*(CB*RR22 - RR21*SB)**2/CW2 &
			& * ( (MH22**2 + MZ2**2 + MA02**2 - 2D0*MH22*MZ2 - 2D0*MH22*MA02 - 2D0*MZ2*MA02)/(4D0*MZ2) )
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
 ...
 A0toH2Z0CT = totalAmplitude
end function A0toH2Z0CT
