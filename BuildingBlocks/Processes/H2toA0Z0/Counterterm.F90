double precision function H2toA0Z0CT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 ...
 select case (x)
	case (1)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Usual()/MZ2 - dMW2Usual()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta1KanUsual() - RR21*CB*dBeta1KanUsual() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSUsual()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSUsual()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSUsual()/2D0 ) &
			&) * (EL2/SW2/CW2)*(CB*RR22 - RR21*SB)**2 * &
			&( (MA02**2 + MZ2**2 + MH22**2 - 2D0*MA02*MZ2 - 2D0*MA02*MH22 - 2D0*MZ2*MH22)/(4D0*MZ2) )
	case (2)
		dRR11 = -CA2*SA1*dAlpha1KanUsual() - CA1*SA2*dAlpha2KanUsual()
		dRR12 = CA1*CA2*dAlpha1KanUsual() - SA1*SA2*dAlpha2KanUsual()
		dRR13 = CA2*dAlpha2KanUsual()
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Usual()/MZ2 - dMW2Usual()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta2KanUsual() - RR21*CB*dBeta2KanUsual() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSUsual()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSUsual()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSUsual()/2D0 ) &
	...
	case (3)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta1KanAlter() - RR21*CB*dBeta1KanAlter() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSAlter()/2D0 ) &
			&) * (EL2/SW2/CW2)*(CB*RR22 - RR21*SB)**2 * &
			&( (MA02**2 + MZ2**2 + MH22**2 - 2D0*MA02*MZ2 - 2D0*MA02*MH22 - 2D0*MZ2*MH22)/(4D0*MZ2) )
	case (4)
		dRR11 = -CA2*SA1*dAlpha1KanAlter() - CA1*SA2*dAlpha2KanAlter()
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta2KanAlter() - RR21*CB*dBeta2KanAlter() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSAlter()/2D0 ) &
	...
	case (5)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta1PinchPStar() - RR21*CB*dBeta1PinchPStar() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSAlter()/2D0 ) &
	...
	case (6)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta2PinchPStar() - RR21*CB*dBeta2PinchPStar() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSAlter()/2D0 ) &
	...
	case (7)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta1PinchOS() - RR21*CB*dBeta1PinchOS() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSAlter()/2D0 ) &
	...
	case (8)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBeta2PinchOS() - RR21*CB*dBeta2PinchOS() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSAlter()/2D0 ) &
	...
	case (9)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Usual()/MZ2 - dMW2Usual()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBetaMSBarUsual() - RR21*CB*dBetaMSBarUsual() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSUsual()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSUsual()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSUsual()/2D0 ) &
	...
	case (10)
		...
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dZA0A0OS()/2D0 + dZZ0Z0OS()/2D0 + (dMZ2Alter()/MZ2 - dMW2Alter()/MW2)/2D0 + &
			& 1D0/(CB*RR22 - RR21*SB)*( CB*dRR22 - dRR21*SB - SB*RR22*dBetaMSBarAlter() - RR21*CB*dBetaMSBarAlter() ) + &
			& (SB*RR22 + RR21*CB)/(CB*RR22 - RR21*SB)*( dZG0A0OSAlter()/2D0 ) + &
			& (CB*RR12 - RR11*SB)/(CB*RR22 - RR21*SB)*( dZH1H2OSAlter()/2D0 ) + &
			& (CB*RR22 - RR21*SB)/(CB*RR22 - RR21*SB)*( dZH2H2OS()/2D0 ) + &
			& (CB*RR32 - RR31*SB)/(CB*RR22 - RR21*SB)*( dZH3H2OSAlter()/2D0 ) &
 ...
 H2toA0Z0CT = totalAmplitude
end function H2toA0Z0CT
