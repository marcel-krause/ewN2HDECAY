...
#include "looptools.h"
 ...
 amplitudes(6) = (0.015625D0*EL2*ML2*(A0(MW2) + (-1.D0*MW2 + x)*B0(x, 0.D0, MW2))*DBLE(YukS3Lep1**INT(2.D0)))/(MW2*PI2*SW2*x)

 amplitudes(7) = (0.015625D0*EL2*ML2*(A0(MHp2) + (-1.D0*MHp2 + x)*B0(x, 0.D0, MHp2))*DBLE(YukS3Lep2**INT(2.D0)))/(MW2*PI2*SW2*x)

 amplitudes(8) = (0.0625D0*EL2*(-1.D0*x - 1.D0*A0(ML2) + ML2*B0(x, 0.D0, ML2) + x*B0(x, 0.D0, ML2)))/(PI2*x)

 amplitudes(9) = (0.015625D0*EL2*(-4.D0*x*DBLE(SW**INT(4.D0)) - 4.D0*A0(ML2)*DBLE(SW**INT(4.D0)) + 4.D0*A0(MZ2)*DBLE(SW**INT(4.D0&
